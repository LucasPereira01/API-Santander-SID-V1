...
def create_segmento_data_base():
    ...
    # Verificar se o usuário não tem permissão para criar o segmento
    if 'santander5' not in sas_user_groups and 'santander4' not in sas_user_groups:
        return jsonify({'error': "Usuario não tem permissão para criar o segmento","campos_error":["Usuario sem Permissão"]}), 403
